...
Analyze Each Matrix Entry
     ...
     +-- C++ Environment (e.g. MSVC, LLVM, Ninja, CUDA)
     +-- clang-format
     +-- CMake Configure (via CMakePresets.json)
     +-- Post-process compile commands
     +-- CodeQL Init [optional]
     +-- Build (via the matching build preset)
     +-- clang-tidy
...
Outside `.github/`, the repository root also contains:
...
`CMakePresets.json` is the single source of truth for build configuration: it defines one preset per OS/build-type combination (e.g. `windows-debug`, `linux-release`), each carrying its own `binaryDir`, compiler, and any project-specific cache variables. Both local development and every CI action configure and build through these presets, so there is no second, independently-maintained copy of the configuration to drift out of sync.
...
**By default this file does not exist**, and none of the actions below require it. It is a purely optional, repository-specific hook, kept outside `.github/` rather than alongside the generic actions, and consumed only by convention (see the `post-process-compile-commands` action below). A repository adds it only if its actual build compiler generates a compilation database that differs from what an analysis tool's Clang-based frontend expects — most commonly a CUDA repository whose database records NVCC command lines. A repository with no such mismatch never needs this file, and the generic `.github/` infrastructure behaves identically whether it exists or not.
...
* A change to `CMakeLists.txt` or `CMakePresets.json` → **CI runs**
...
The `configure-cmake` action normalizes the matrix `config` value to lowercase and configures the project by invoking the matching preset from `CMakePresets.json` — `windows-<config>` or `linux-<config>` — rather than reconstructing compiler flags itself:
...
Because each preset has its own `binaryDir`, the action exposes the resolved directory as a `build-dir` output so later steps don't need to re-derive the preset naming convention:
...
#### 6. Post-process compile commands

The `post-process-compile-commands` action takes the preset's `build-dir` (from the configure step's output) and:

* Locates and validates `<build-dir>/compile_commands.json`.
* Reports the number of compilation entries it contains.
* Checks whether the repository provides `scripts/post_process_compile_commands.py`.
* If the script exists, invokes it and validates the resulting database, including that the entry count is unchanged.
* **If the script does not exist — the default for most repositories — copies the database unchanged.** The action itself has no compiler- or language-specific knowledge; it behaves identically for every repository regardless of whether the optional script is present.
* Exposes the resulting directory as a `db-dir` output, consumed by the `run-clang-tidy` step.

This step exists because `clang-tidy` does not execute the compiler recorded in `compile_commands.json` — it feeds the recorded arguments to Clang's own frontend. For most C++ projects this database is already Clang-compatible and the copy-unchanged path is all that ever runs. A repository whose actual build compiler diverges from that — for example, a CUDA repository whose database records NVCC command lines, several of which are meaningless or actively misleading to Clang — can add `scripts/post_process_compile_commands.py` to translate or remove them before `clang-tidy` runs, without this action or the workflow needing any awareness of why. The original database is always left untouched for the actual build.
...
The `build-project` action builds the project via the matching build preset — `cmake --build --preset <os>-<config>` — rather than a hardcoded build directory, so it stays correct regardless of which preset `configure-cmake` just configured.

#### 9. Run clang-tidy

The `run-clang-tidy` action performs the clang-tidy analysis using the post-processed compilation database (`db-dir` from step 6).
...
#### 12. Upload Compilation Databases

Both the original and post-processed compilation databases (located via the `configure-cmake` and `post-process-compile-commands` step outputs, respectively) are uploaded as a separate artifact and retained for seven days.
...
Configures the project by invoking the CMake preset matching the current OS and the (lowercase-normalized) `config` input, rather than passing compiler/build-type flags directly:

```text
cmake --preset windows-<config>   # Windows, after initializing the MSVC dev environment
cmake --preset linux-<config>     # Linux
```

All compiler, generator, and build-type selection lives in `CMakePresets.json`; this action's only job is picking the right preset name and initializing the MSVC environment on Windows beforehand. It exposes the preset's binary directory as a `build-dir` output.
...
Takes a `build-dir` input (the directory containing `compile_commands.json`, normally the `configure-cmake` action's `build-dir` output) and produces an analysis-ready compilation database:

* Validates the source database exists and is well-formed JSON.
* Checks for an optional repository-specific `scripts/post_process_compile_commands.py`.
* Invokes it if present, or copies the database unchanged if not.
* Validates the result, including that the number of entries is unchanged.
* Exposes the resulting directory as a `db-dir` output.

This action contains no compiler- or language-specific knowledge — it is safe to run for any repository. By default no such script exists, and the action simply copies the database through unchanged. Compiler-specific translation (for example, adapting NVCC-generated CUDA compilation commands for `clang-tidy`'s Clang-based frontend, for a repository that has that mismatch) lives entirely in the optional `scripts/post_process_compile_commands.py`, kept outside `.github/` so this action stays identical across repositories regardless of whether any particular one needs that customization.
...
Builds the project via the matching CMake build preset:
...
Like `configure-cmake`, it normalizes `config` to lowercase before selecting the preset, so the binary directory always matches whatever `configure-cmake` just configured.
...
Runs clang-tidy against the project using the post-processed compilation database (`post-process-compile-commands`'s `db-dir` output).
...
The individual components can also be run locally using the project's normal CMake preset workflow — the same `CMakePresets.json` the CI actions consume:
...
CMakePresets.json
    |
    +-- What configuration to build (compiler, build type, architecture)?
```
...
Operating-system-specific implementation belongs in composite actions and `CMakePresets.json` wherever practical, not hardcoded into the workflow or reconstructed independently by CI.
...
### Compiler-Specific Knowledge Stays Out of `.github/`

Where an analysis tool's requirements differ from the actual build compiler's dialect (for example, `clang-tidy` consuming NVCC-generated CUDA commands in a repository that has one), the translation lives in an optional, repository-specific script outside `.github/` (`scripts/post_process_compile_commands.py`), never in the generic actions or workflow themselves. Most repositories need no such script at all — it is absent by default, and its absence is the normal case, not a missing setup step.
...
6. Keep platform-specific behavior inside the appropriate composite action or `CMakePresets.json`.
7. Avoid introducing implicit configuration defaults.
8. If a repository needs compiler-specific compilation-database translation, keep it inside that repository's optional `scripts/post_process_compile_commands.py`, never in `.github/`. Most repositories will not need this file at all.

When adding a new analysis capability, first determine whether it belongs in configuration, workflow orchestration, a composite action, or `CMakePresets.json` before modifying the pipeline.
